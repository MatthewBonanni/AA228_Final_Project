...
class RaceNetBranched(torch.nn.Module):
    def __init__(self, args, num_drivers, num_tracks, num_teams):
        ...
        in_cons = args["emb_dim"]*len(ids) + 1
    ...
    def forward(self, batched_lap, batched_cons, batched_weath, batched_events):
        beds = []
        for i, embed in enumerate(self.embs):
            beds.append(embed(batched_cons[:,i]))         
        input_cons = batched_cons[:,[-1]]
        for bed in beds:
...
class RaceNetBranchedPretrainer(torch.nn.Module):
    ...
    def forward(self, batched_lap, batched_cons, batched_weather, batched_events):
        beds = []
        for i, embed in enumerate(self.embs):
            beds.append(embed(batched_cons[:,i]))         
        input_cons = batched_cons[:,[-1]]
...
class MHRaceNetBranched(torch.nn.Module):
    def __init__(self, args, num_drivers, num_tracks, num_teams, activation=F.silu):
        ...
        in_laps = len(lap_cols) + len(events) + 1
        in_cons = len(cols) - len(ids) - len(lap_cols) + args["emb_dim"]*len(ids) + 2
        in_weath = len(weather_cols)
    ...
    def forward(self, batched_lap, batched_cons, batched_weath, batched_events):
        ...
        input_cons = input_cons.to(torch.float32)
        input_weath = batched_weath.to(torch.float32)
        input_lap = torch.cat((batched_lap,batched_events),dim=-1).to(torch.float32)
...
class RaceNet(torch.nn.Module):
    ...
    def forward(self, batched_lap, batched_cons, batched_weath, batched_events):
        ...
        for i, embed in enumerate(self.embs):
            beds.append(embed(batched_cons[:,i]))         
        input = torch.cat((batched_lap, batched_events, batched_cons[:,[-1]], batched_weath), dim=-1)
        for bed in beds:
...
class F1Dataset(Dataset):
    def __init__(self, data):
        ...
        self.inputs.loc[:,'Year'] = self.inputs.loc[:,'Year'] - self.inputs['Year'].min()
    ...
    def __getitem__(self, idx):
        ...
        input_ids = np.concatenate((input_ids,[time_dat]))
        ...
        return input_lap, input_ids, input_weath, input_events, label
...
def train(dataloader, model, optimizer, scheduler, loss_fn=F.mse_loss):
    ...
    for step, batch in enumerate(tqdm(dataloader, desc="Iteration")):
        ...
        out = model(batch[0],batch[1],batch[2], batch[3]).squeeze()
...
if __name__=="__main__":
    ...
    now = datetime.now()
    filename = 'outputs/racenet_pt_' + now.strftime('%m_%d_%H_%M') +"_"+ str(args["num_layers"]) + "l_" + str(args["hidden_dim"]) + '.pt'
