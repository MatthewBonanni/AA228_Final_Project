...
class RaceNetBranched(torch.nn.Module):
    ...
    def forward(self, batched_lap, batched_cons, batched_weath, batched_ids):
        ...
        output = (input_lap*output).sum(dim=-1)
        return output  
...
class F1Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_lap = np.concatenate([input_lap,[two_sector_time]])
...
if __name__=="__main__":
    ...
    now = datetime.now()
    filename = 'outputs/racenet_' + now.strftime('%m_%d_%H_%M') +"_"+ str(args["num_layers"]) + "l_" + str(args["hidden_dim"]) + '.pt'
